...
For this article we will use the synthetic biology patent dataset developed by Paul Oldham based on a simple search of the Thomson Innovation database using key terms. The dataset consists of 576 first filings of patent applications linked to 2,882 Family members worldwide. The dataset has been extensively cleaned and is described in detail in the codebook [here](). The dataset serves to provide an insight into a new and emerging areas of science and technology in training exercises but does not provide the full patent landscape. 
