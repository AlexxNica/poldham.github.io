...
The Patentscope data does not contain a publication country field. Note here that Patentscope merges all publications for an application record into one dossier. So we are only seeing one document of a set of documents (in Patentscope the wider dossier is accessible through the Documents section of the website). However, we could access this information from the front of the publication number in the Patentscope records using a very simple code and create a new column (as above) based on the values returned as below.
